/**
 * Thought Line — adapted from React Bits
 * https://reactbits.dev/micro/thought-line (DavidHDev/react-bits).
 * Icons via @tabler/icons-react; motion via motion/react.
 */
...
export function ThoughtLine({
  ...
  if (onSettle) latest.current = { onSettle };
  else latest.current = {};
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: glyph toggles the glyph DOM node; re-bind breath when it appears
  useEffect(() => {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: label/doneText/fontSize/showTimer change measured widths
  useLayoutEffect(() => {
  ...
  }, [isWorking, label, doneText, showTimer]);
  ...
  const head = (
    <>
      {glyph !== "none" ? (
        ...
        >
          {glyph === "sparkle" ? (
            <IconSparkles size="100%" stroke={2} />
          ) : glyph === "dot" ? (
      ...
      {collapsible ? (
        <span
          ...
          aria-hidden="true"
        >
          <IconChevronDown size="1em" stroke={2.2} />
        </span>
      ) : null}
  ...
  return (
    <div
      className={`group inline-flex flex-col items-start font-medium leading-[1.2] [color:var(--tl-color)] [font-family:inherit] [font-size:var(--tl-font)]${className ? ` ${className}` : ""}`}
    ...
    >
      ...
      {collapsible ? (
        <button
          type="button"
          className="relative m-0 inline-flex cursor-default items-center gap-[0.3em] border-0 bg-transparent p-0 text-left text-inherit whitespace-nowrap outline-none [font:inherit] data-[toggle]:cursor-pointer data-[toggle]:[-webkit-tap-highlight-color:transparent]"
        ...
        <div className="relative m-0 inline-flex cursor-default items-center gap-[0.3em] border-0 bg-transparent p-0 text-left text-inherit whitespace-nowrap outline-none [font:inherit] data-[toggle]:cursor-pointer data-[toggle]:[-webkit-tap-highlight-color:transparent]">
          {head}
        </div>
      )}
      {hasTrace ? (
